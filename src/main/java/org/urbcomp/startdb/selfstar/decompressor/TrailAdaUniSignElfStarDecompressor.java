...
import org.urbcomp.startdb.selfstar.decompressor.xor.TrailElfStarXORDecompressor;
...
public class TrailAdaUniSignElfStarDecompressor implements IDecompressor {
    private final TrailElfStarXORDecompressor xorDecompressor;
    ...
    public TrailAdaUniSignElfStarDecompressor(TrailElfStarXORDecompressor xorDecompressor) {
    ...
    public List<Double> decompress() {
        ...
        for (int i=0;i<signLength;i++){
            signValue = readInt(1);
            signCnt = signCntWidth == 0 ? 1 : readInt(signCntWidth);
...
//        }
        ...
        xorDecompressor.initLeadAndTrailRepresentation();

        // trailingZeros
        trailingZeros = new int[numberOfValues];
        int trailingZerosLength = readInt(32);
        int trailingZerosCntWidth = readInt(32);
        int trailingZeroWidth = readInt(32);

        int trailingZerosValue,trailingZerosCnt,trailingZerosIdx=0;
        for (int i=0;i<trailingZerosLength;i++){
            trailingZerosValue = readInt(trailingZeroWidth);
            trailingZerosCnt = trailingZerosCntWidth == 0 ? 1 : readInt(trailingZerosCntWidth);
            for (int j=0;j<trailingZerosCnt;j++){
                trailingZeros[trailingZerosIdx++] = trailingZerosValue;
            }
        }
        xorDecompressor.setTrailingZeros(trailingZeros);
