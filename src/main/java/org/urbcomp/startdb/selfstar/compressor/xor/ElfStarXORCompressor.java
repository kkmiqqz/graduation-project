...
public class ElfStarXORCompressor implements IXORCompressor {
    ...
    @Override
    public int addValue(long value, int noEraseFlag) {
        if (first) {
            return initLeadingRoundAndRepresentation(leadDistribution)
                    + initTrailingRoundAndRepresentation(trailDistribution)
                    + writeFirst(value);
        } else {
            return compressValue(value, noEraseFlag);
        }
    }
    ...
    private int compressValue(long value, int noEraseFlag) {
        int thisSize = 0;
        long xor = storedVal ^ value;
