...
import org.urbcomp.startdb.selfstar.compressor.xor.TrailElfStarXORCompressor;
 ...
 */
public class TrailAdaUniSignElfStarCompressor implements ICompressor {
    private final TrailElfStarXORCompressor xorCompressor;
...
//    private int lastBetaStar = Integer.MAX_VALUE;
    ...
    public TrailAdaUniSignElfStarCompressor(TrailElfStarXORCompressor xorCompressor, int window) {
...
//        this.betaStarList = new int[window + 1];     // one for the end sign
    ...
    public TrailAdaUniSignElfStarCompressor(TrailElfStarXORCompressor xorCompressor) {
        this.xorCompressor = xorCompressor;
        this.os = xorCompressor.getOutputStream();
//        this.betaStarList = new int[1001];     // one for the end sign
    ...
    public void AdaUnify(List<Double> dbs){
        ...
        trailingZeros = new int[dbs.size()];
        int idx=0;
        for (double db:dbs) {
...
//            }
    ...
    private void calculateDistribution() {
        ...
        for (int i = 1; i < numberOfValues; i++) {
            ...
            if (xor != 0) {
                tmp = Long.numberOfTrailingZeros(xor);
                trailingZeros[i] = tmp;
                trailDistribution[tmp]++;
...
//        }
        ...
        compressedSizeInBits += xorCompressor.initLeadingAndTrailing();

        xorCompressor.encodeTrailingZero(trailingZeros);

        // trailingZeros
        RLEresult trailingZerosResult = AdaRLE(trailingZeros, xorCompressor.getTrailingBitsPerValue());

        // 序列化 trailingZeros
        compressedSizeInBits += os.writeInt(trailingZerosResult.length,32); // TODO: 可隐含
        compressedSizeInBits += os.writeInt(trailingZerosResult.cntWidth,32);
        compressedSizeInBits += os.writeInt(trailingZerosResult.itemWidth,32);

        for (int i=0;i<trailingZerosResult.length;i++){
            compressedSizeInBits += os.writeInt(trailingZerosResult.nums[i],xorCompressor.getTrailingBitsPerValue());
            compressedSizeInBits += os.writeInt(trailingZerosResult.cnts[i],trailingZerosResult.cntWidth);
        }
