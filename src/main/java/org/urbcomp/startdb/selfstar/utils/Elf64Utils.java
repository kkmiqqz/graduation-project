...
public class Elf64Utils {
    ...
    private final static double sweetVlaue = Math.pow(2,52)+Math.pow(2,51);
    ...
    /**
     * 纯计算的小数位数计算函数，较字符串方法速度有大幅提升，但是存在误判的情况，可能会将精度估小
     * @param v 小数
     * @return 小数精度
     */
    public static int getAlpha(double v) {
        double log10v = Math.log10(v);
        int sp = (int) Math.floor(log10v);
        int i;
        if (sp >= 0) {
        ...
        double temp;
//        while ((long) temp != temp) { // 直到没有小数位为止
        while (true) { // 直到没有小数位为止
            temp = v * get10iP(i);
//            double dtmp = Math.round(temp);
            double dtmp = temp + sweetVlaue - sweetVlaue;
            long ldtmp = Double.doubleToRawLongBits(dtmp);
            long ltemp = Double.doubleToRawLongBits(temp);
            if (Math.abs(ldtmp-ltemp)<=2)
                break;
            i++;
        }
        return i;
...
//        while ((long) temp != temp) { // 直到没有小数位为止
    ...
    public static void main(String[] args) {
        double test = 0.5099210882371499;
        int alpha = getAlpha(test);
    }
